...
**You run all four scripts. Analysis complete!** ✅
...
❌ Keep dependencies in your head (error-prone!)  
❌ Re-run everything every time (slow!)  
...
✅ **Solution: GNU Make automatically tracks all dependencies!**
...
- Only re-runs what changed (edit plot.R → 2 min rerun!)
...
**⚠️ IMPORTANT:** Commands MUST start with a TAB character, not spaces!
...
2. Does `raw_data.csv` exist?  
   - No → Error: can't build target
   - Yes → Go to question 3

. . .

3. Is `raw_data.csv` newer than `clean_data.csv`?  
   - Yes → Run the command (rebuild needed)
   - No → Skip! Target is up to date
...
- Make sees `clean.R` changed → rebuilds `clean_data.csv`!
...
```
raw_data.csv → clean_data.csv → statistics.txt → plot.png
```
...
This one rule handles ALL csv → xlsx conversions!
...
- Multi-step pipeline (download → clean → analyze → visualize)
