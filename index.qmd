...
❌ Re-run everything (slow!)

. . .

❌ Manually track what needs updating (error-prone!)

. . .

❌ Forget to update downstream files (wrong results!)

. . .

✅ **Solution: GNU Make automatically tracks dependencies!**


## What is GNU Make? 🔧

["GNU is not unix"]{.primary}, a build automation tool that:
...
- Make is pretty steep. It’s frustrating to learn, and it can be tricky to use even after you’ve learned the basics. 
...
`raw_data.csv` → `clean_data.csv` → `statistics.txt`
...
```
raw_data.csv → clean_data.csv → statistics.txt → plot1.png
```
...
1. Check if `data/raw_data.csv` exists → download if needed
2. Check if `data/clean_data.csv` is up-to-date → clean if needed
3. Check if `results/statistics.txt` is up-to-date → analyze if needed
4. Check if `figures/plot1.png` is up-to-date → plot if needed
...
This rule works for ANY csv → xlsx conversion!
...
✅ **Make tracks dependencies** between files

. . .

✅ **Only rebuilds what's necessary** (saves time!)

. . .

✅ **Documents your workflow** in one place

. . .

✅ **Works with any tools** (R, Python, bash, etc.)
...
**Makefile = Your research lab notebook** 📓
...
**Troubleshooting:**
- If `make` command not found → Restart terminal/PowerShell
...
**Troubleshooting:**
- If `make --version` shows older version → Use `brew install make`
- Homebrew installs as `gmake` on some systems → Create alias or use `gmake`
